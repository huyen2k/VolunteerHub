...
export default function DashboardPage() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [statsRes, unreadNoti, topNewEvents, hotPosts, attractiveRes] =
        await Promise.all([
      ...
      const maxScore =
        mappedAttr.length > 0 ? Math.max(...mappedAttr.map((e) => e.score)) : 1;
        ...
        <div className="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
          <div className="mb-8">
            ...
            <p className="mt-1 text-muted-foreground">
              Chào mừng trở lại! Cập nhật tin tức và hoạt động mới nhất.
            </p>
          </div>
          ...
          <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Card
              ...
              onClick={() =>
                navigate("/user/events", { state: { filter: "registered" } })
              }
            >
            ...
            <Card
              ...
              onClick={() =>
                navigate("/user/events", { state: { filter: "upcoming" } })
              }
            >
              <CardContent className="p-6">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <div className="mt-1 flex items-center text-xs text-orange-600 font-medium bg-orange-50 px-2 py-0.5 rounded-full w-fit">
                      <PlayCircle className="h-3 w-3 mr-1" /> Đã đăng ký
                    </div>
                  </div>
            ...
            <Card
              className="cursor-pointer hover:shadow-lg transition-all border-l-4 border-l-green-600 hover:-translate-y-1 bg-white"
              onClick={() =>
                navigate("/user/events", { state: { filter: "my-happening" } })
              }
            >
              <CardContent className="p-6">
                <div className="flex justify-between items-center">
            ...
            <Card
              className="cursor-pointer hover:shadow-lg transition-all border-l-4 border-l-pink-500 hover:-translate-y-1 bg-white"
              ...
              <CardContent className="p-6">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <div className="mt-1 flex items-center text-xs text-pink-600 font-medium bg-pink-50 px-2 py-0.5 rounded-full w-fit">
                      <Bell className="h-3 w-3 mr-1" /> Cần xem
                    </div>
                  </div>
            ...
            <div className="lg:col-span-2 space-y-8">
              <div>
                <h2 className="text-lg font-bold flex items-center gap-2 mb-4">
                  <Sparkles className="h-5 w-5 text-yellow-500" /> Sự kiện mới
                  công bố
                </h2>
                    ...
                    {newlyPublishedEvents.length > 0 ? (
                          ...
                          >
                            <div>
                              ...
                              <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                                ...
                                <span className="flex items-center gap-1">
                                  <Users className="h-3 w-3" /> {event.location}
                                </span>
                              </div>
                            </div>
                      ...
                      <div className="text-center py-12">
                        ...
                        <p className="text-muted-foreground">
                          Không có sự kiện mới nào trong 7 ngày qua.
                        </p>
                      </div>
                    )}
                  </CardContent>
              ...
              <div>
                <h2 className="text-lg font-bold flex items-center gap-2 mb-4">
                  <MessageSquare className="h-5 w-5 text-blue-500" /> Thảo luận
                  nổi bật
                </h2>
                <div className="space-y-3">
                  {recentDiscussions.length > 0 ? (
                    recentDiscussions.map((item, idx) => (
                      ...
                      >
                        ...
                        <Button
                          size="sm"
                          variant="ghost"
                          className="text-blue-600 shrink-0 hover:text-blue-700 hover:bg-white/50 rounded-full"
                        >
                          <ArrowRight className="h-4 w-4" />
                        </Button>
                      </div>
                    ...
                    <div className="text-center py-10 bg-white rounded-xl border border-dashed border-gray-200">
                      <p className="text-muted-foreground">
                        Chưa có thảo luận nào sôi nổi gần đây.
                      </p>
                    </div>
                  )}
                </div>
              ...
              <Card className="h-full border-none shadow-sm sticky top-6">
                <CardHeader className="pb-3 border-b bg-gray-50/50">
                  <CardTitle className="flex items-center gap-2 text-base font-bold">
                    <TrendingUp className="h-5 w-5 text-purple-600" /> Top sự
                    kiện thu hút
                  </CardTitle>
                </CardHeader>
                <CardContent className="pt-4 space-y-4">
                  {attractiveEvents.length > 0 ? (
                    attractiveEvents.map((event, idx) => (
                      <div
                        key={event.id}
                        className="flex items-start gap-3 pb-3 border-b last:border-0 last:pb-0 cursor-pointer group"
                        onClick={() => navigate(`/events/${event.id}`)}
                      >
                        <div
                        ...
                        <div className="flex-1">
                          ...
                          <div className="mt-2 h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${Math.min(
                                  (event.score / maxAttractiveScore) * 100,
                                  100
                                )}%`,
                                minWidth: "5%",
                              }}
