...
export default function UserEventsPage() {
  ...
  useEffect(() => {
    if (location.state?.filter && !processedFilterRef.current) {
      ...
      else if (filterType === "my-happening") {
        setFilterDate("happening");
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        ...
        const rawEvents = Array.isArray(pageData.content)
          ? pageData.content
          : [];
        setEvents(rawEvents);
  ...
  useEffect(() => {
    ...
    let result = sourceList.filter((event) => {
      ...
      if (filterDate !== "all") {
        ...
        if (filterDate === "happening" && (now < eventDate || now > endDate))
          return false;
  ...
  const getStatusBadge = (event, isFull, isHappening, isEnded) => {
    if (event.isRegistered) {
      if (event.registrationStatus === "completed")
        return (
          <Badge className="bg-purple-100 text-purple-700">Hoàn thành</Badge>
        );
      ...
      if (event.registrationStatus === "approved" && isEnded)
        return (
          <Badge className="bg-blue-100 text-blue-700">
            <Hourglass className="w-3 h-3 mr-1" /> Đợi xác nhận hoàn thành
          </Badge>
        );

      if (event.registrationStatus === "approved")
        return (
          <Badge className="bg-green-100 text-green-700">Đã xác nhận</Badge>
        );
      if (event.registrationStatus === "rejected")
        return <Badge className="bg-red-100 text-red-700">Bị từ chối</Badge>;
      return (
        <Badge className="bg-yellow-100 text-yellow-700">
          <Hourglass className="w-3 h-3 mr-1" /> Chờ duyệt
        </Badge>
      );
    }
          ...
          <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
            ...
            {user && (
              <div className="flex gap-3 items-center">
                ...
                <Badge
                  variant="outline"
                  className="text-sm py-2 px-3 bg-white border-gray-200"
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="absolute top-3 right-3">
                        {getStatusBadge(event, isFull, isHappening, isEnded)}
                      </div>
                        ...
                        <div className="pt-2">
                          <div className="flex justify-between text-xs mb-1">
                            <span className="font-medium text-gray-700 flex items-center gap-1">
                              <Users className="w-3 h-3" /> {currentQty} tham
                              gia
                            </span>
                            <span className="text-gray-500">
                              Mục tiêu: {maxQty > 0 ? maxQty : "∞"}
                            </span>
                          </div>
                          <div className="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${
                                  maxQty > 0
                                    ? Math.min((currentQty / maxQty) * 100, 100)
                                    : 0
                                }%`,
                              }}
                            ></div>
